// 圖標系統 — Lucide 線稿 SVG 內嵌字串（設計系統 §4：零 CDN、viewBox 24、
// stroke: currentColor、stroke-width 1.75、預設顯示 16×16）。
// 用法：
//   1. 靜態 HTML：<span data-icon="home"></span> → initOverlays 時 mountIcons() 填入
//   2. 動態改字的按鈕：btn.innerHTML = `${iconHtml('eye')}<span>視角：…</span>`
// 鉻件（按鈕 / HUD 標題 / 工具列）一律用這裡的 SVG，禁用 emoji。
...
/** 產生單顆 SVG 圖標字串（stroke: currentColor → 跟著文字色走） */
...
/** 把文件中所有 <span data-icon="xxx"> 填入對應 SVG（靜態 HTML 的圖標掛載） */
